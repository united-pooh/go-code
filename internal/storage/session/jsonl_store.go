...
type JSONLStore struct {
	...
	legacyGlobalBaseDir string // HOME/.paw/projects/<同一项目 ID>，优先于工作区旧布局
...
// 新数据写入 PAW_CONFIG_HOME/projects/<项目名>/sessions/（默认 ~/.paw）。
// 旧默认全局项目目录、旧工作区 .paw 依次作为只读 fallback。
func NewJSONLStoreForWorkspace(workspaceRoot string) (*JSONLStore, error) {
	...
	projectDir, err := pawpath.ProjectDir(workspaceRoot)
	if err != nil {
		return nil, err
	}
	...
	if err != nil {
		return nil, fmt.Errorf("获取旧全局目录失败: %w", err)
	}
	store.legacyGlobalBaseDir, err = pawpath.ProjectDirInHome(filepath.Join(home, ".paw"), workspaceRoot)
	if err != nil {
		return nil, err
	}
...
// resolveSessionDir 以 meta.json 为发布标记，统一选择完整会话的读根。
// 只有不存在才 fallback；权限或损坏错误交给实际读取返回，不隐藏高优先级数据。
...
func (s *JSONLStore) ensureWritableSession(sessionID string) (bool, error) {
	...
	for _, root := range s.sessionRoots() {
		...
		var meta Meta
		if err := json.Unmarshal(data, &meta); err != nil {
			return false, fmt.Errorf("解析 meta.json 失败: %w", err)
		}
		if dir != global {
			if err := s.migrateSession(dir, global, data); err != nil {
...
// TranscriptPath 返回指定 session 的可读 transcript 文件路径。
func (s *JSONLStore) TranscriptPath(sessionID string) string {
	return s.readTranscriptPath(sessionID)
}
...
// TurnMetadataPath returns the readable sidecar path for persisted turn timing.
func (s *JSONLStore) TurnMetadataPath(sessionID string) string {
	return s.readTurnMetadataPath(sessionID)
}
...
func (s *JSONLStore) readKeyIndex(key string) (string, error) {
	name := filepath.Base(s.keyIndexPath(key))
	for _, root := range s.storageRoots() {
		if data, err := os.ReadFile(filepath.Join(root, "index", name)); err == nil {
...
// storageRoots 只包含同一工作区的精确项目路径，不扫描 projects 下的其它项目。
func (s *JSONLStore) storageRoots() []string {
	var roots []string
	seen := make(map[string]bool)
	for _, root := range []string{s.baseDir, s.legacyGlobalBaseDir, s.legacyBaseDir} {
...
func (s *JSONLStore) sessionRoots() []string {
	...
	for i := range roots {
		roots[i] = filepath.Join(roots[i], defaultSessionsDir)
	}
	return roots
}
