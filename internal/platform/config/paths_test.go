...
func TestResolvePathsUsesExplicitConfigAndWorkspaceRoots(t *testing.T) {
	...
	projectDir, err := pawpath.ProjectDirInHome(configHome, workspace)
	if err != nil {
		t.Fatal(err)
	}
	...
	wants := map[string]string{"home": configHome, "config": filepath.Join(configHome, "config.jsonc"), "settings": filepath.Join(configHome, "settings.json"), "mcp": filepath.Join(configHome, "mcp.toml"), "skills": filepath.Join(configHome, "skills"), "schema": filepath.Join(configHome, "schemas", "config-v2.schema.json"), "workspace": filepath.Join(projectDir, "config.jsonc")}
