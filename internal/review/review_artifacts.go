...
const summaryFileName = "last-run-summary.json"
...
func WriteReviewArtifacts(ctx context.Context, opts ReviewArtifactsOptions) (ReviewArtifactsResult, error) {
	...
	if err != nil {
		return ReviewArtifactsResult{}, fmt.Errorf("resolve review artifacts: %w", err)
	}
	artifactDir := filepath.Join(project, "artifacts", "review")
	if err := os.MkdirAll(artifactDir, 0o700); err != nil {
		return ReviewArtifactsResult{}, fmt.Errorf("create review artifact directory: %w", err)
	}

	readme, err := readProjectDocumentation(root)
	if err != nil {
		return ReviewArtifactsResult{}, err
	}
	...
	generatedFiles := []string{
		filepath.Join(artifactDir, "design.md"),
		filepath.Join(artifactDir, "spec.json"),
		filepath.Join(artifactDir, "plan.json"),
		filepath.Join(artifactDir, "architecture.json"),
		filepath.Join(artifactDir, "dispatch.json"),
		filepath.Join(artifactDir, "execution-report.json"),
		filepath.Join(artifactDir, "merge-report.json"),
		filepath.Join(artifactDir, "validation-report.json"),
		filepath.Join(artifactDir, "tree-classification.json"),
		filepath.Join(artifactDir, "tree-rubrics.json"),
		filepath.Join(artifactDir, "tree-rubric-verification.json"),
		filepath.Join(artifactDir, "tree-rubrics-refined.json"),
		filepath.Join(artifactDir, "tree-grading-individual.json"),
		filepath.Join(artifactDir, "qa-report.json"),
		filepath.Join(artifactDir, "doc-report.json"),
		filepath.Join(artifactDir, "final-assessment.json"),
		filepath.Join(artifactDir, "status.json"),
		filepath.Join(artifactDir, summaryFileName),
	}
	execution := ExecutionReportArtifact{
		...
		GeneratedFiles: toSlashPaths(artifactDir, generatedFiles),
	}

	if err := writeTextFile(filepath.Join(artifactDir, "design.md"), design); err != nil {
		return ReviewArtifactsResult{}, err
	}
	for path, payload := range map[string]any{
		filepath.Join(artifactDir, "spec.json"):                     spec,
		filepath.Join(artifactDir, "plan.json"):                     plan,
		filepath.Join(artifactDir, "architecture.json"):             architecture,
		filepath.Join(artifactDir, "dispatch.json"):                 dispatch,
		filepath.Join(artifactDir, "execution-report.json"):         execution,
		filepath.Join(artifactDir, "merge-report.json"):             merge,
		filepath.Join(artifactDir, "validation-report.json"):        validation,
		filepath.Join(artifactDir, "tree-classification.json"):      classification,
		filepath.Join(artifactDir, "tree-rubrics.json"):             baseRubrics,
		filepath.Join(artifactDir, "tree-rubric-verification.json"): verification,
		filepath.Join(artifactDir, "tree-rubrics-refined.json"):     refined,
		filepath.Join(artifactDir, "tree-grading-individual.json"):  grading,
		filepath.Join(artifactDir, "qa-report.json"):                qa,
		filepath.Join(artifactDir, "doc-report.json"):               doc,
		filepath.Join(artifactDir, "final-assessment.json"):         assessment,
		filepath.Join(artifactDir, "status.json"): map[string]any{
			"status":       "completed",
			"active":       false,
			"strict":       true,
			"generated_at": now.Format(time.RFC3339),
	...
	summaryPath := filepath.Join(artifactDir, summaryFileName)
...
func shouldSkipDir(name string) bool {
	...
	case ".git", ".paw", ".pipeline-workspace", "vendor", "node_modules", "tmp", "temp", "users":
		return true
	default:
...
func moduleFamily(path string) string {
	...
	switch {
	case strings.HasPrefix(path, "cmd/"), strings.HasPrefix(path, "internal/entry/"):
		return "cli"
	case strings.HasPrefix(path, "internal/runtime/streamma"):
		return "runtime"
	case strings.HasPrefix(path, "internal/runtime/loop"):
		return "orchestration"
	case strings.HasPrefix(path, "internal/runtime/task"):
		return "task"
	// Report families are scoring categories, not directory groups.
	case strings.HasPrefix(path, "internal/ui/web"), strings.HasPrefix(path, "internal/ui/complete"), strings.HasPrefix(path, "internal/ui/theme"):
		return "package"
	case strings.HasPrefix(path, "internal/ui"):
		return "ui"
	case strings.HasPrefix(path, "internal/capability/tool"):
		return "tool"
	case strings.HasPrefix(path, "internal/capability/model"):
		return "model"
	case strings.HasPrefix(path, "internal/storage/session"), strings.HasPrefix(path, "internal/runtime/sessionactor"):
		return "session"
	case strings.HasPrefix(path, "internal/platform/settings"):
		return "settings"
...
func readProjectDocumentation(root string) (string, error) {
	var text strings.Builder
	for _, rel := range []string{"README.md", "docs/README.md", "docs/architecture", "docs/guides", "docs/reference"} {
		...
		if err != nil {
			return "", fmt.Errorf("read project documentation: %w", err)
		}
	}
