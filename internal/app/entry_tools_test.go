...
func TestRegisterToolsSkipsTaskOrchestrationForWorker(t *testing.T) {
	...
	if err := RegisterBuiltinTools(worker, t.TempDir(), nil, nil, "", nil, false, true); err != nil {
		t.Fatal(err)
	}
	...
	if err := RegisterBuiltinTools(main, t.TempDir(), nil, nil, "", nil, false, false); err != nil {
		t.Fatal(err)
	}
	for _, name := range []string{"Task", "TaskStatus", "TaskStop", "TaskWait"} {
...
func TestRegisterToolsDoesNotAddQuestion(t *testing.T) {
	registry := tool.NewRegistry()
	if err := RegisterBuiltinTools(registry, t.TempDir(), nil, nil, "", nil, false, false); err != nil {
		t.Fatal(err)
	}
	if _, ok := registry.Get("question"); ok {
...
func TestRegisterToolsEnablesOutsideReadInDangerousMode(t *testing.T) {
	registry := tool.NewRegistry()
	if err := RegisterBuiltinTools(registry, t.TempDir(), nil, nil, "", nil, true, false); err != nil {
		t.Fatalf("registerTools: %v", err)
	}
