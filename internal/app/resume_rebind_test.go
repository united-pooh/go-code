...
func TestResumeRebindsStateTools(t *testing.T) {
	...
	app := &WorkspaceRuntime{SessionHost: &sessionactor.Host{Engine: runner}, Store: store, Toolset: toolset}
	BindSessionTools(app, broker, "session-a")
	runner.SetSessionLoadedHook(func(sid string) {
		BindSessionTools(app, broker, sid)
	})
...
func TestResumeRestoresTodoBrokerFromSession(t *testing.T) {
	...
	runner.SetSessionLoadedHook(func(sid string) { BindSessionTools(app, broker, sid) })
	if _, err := runner.LoadSession(ctx, "session-b"); err != nil {
		t.Fatal(err)
	}
