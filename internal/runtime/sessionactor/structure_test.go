...
func TestProductionEngineConstructionIsWrappedBySessionHost(t *testing.T) {
	...
	err := filepath.WalkDir(repoRoot, func(path string, entry os.DirEntry, walkErr error) error {
		...
		if !strings.HasSuffix(path, ".go") || strings.HasSuffix(path, "_test.go") || strings.Contains(path, filepath.Join("internal", "runtime", "loop")+string(filepath.Separator)) {
			return nil
		}
		...
		loopAliases := importAliases(file, "paw/internal/runtime/loop", "loop")
		hostAliases := importAliases(file, "paw/internal/runtime/sessionactor", "sessionactor")
