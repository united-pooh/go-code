...
func TestInvariantI6NoDomainImports(t *testing.T) {
	out, err := exec.Command("go", "list", "-deps", "./internal/runtime/actor").Output()
	if err != nil {
		t.Skipf("go list unavailable: %v", err)
	}
	forbidden := []string{
		"paw/internal/runtime/loop", "paw/internal/runtime/task", "paw/internal/runtime/goal",
		"paw/internal/runtime/plan", "paw/internal/storage/session", "paw/cmd",
	}
	for _, line := range strings.Split(string(out), "\n") {
		for _, bad := range forbidden {
			if strings.HasPrefix(line, bad) {
				t.Fatalf("I6 violated: %s imports %s", "internal/runtime/actor", line)
			}
		}
	}
}
