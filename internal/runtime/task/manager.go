...
func NewManager(cfg Config) *Manager {
	...
	m.actors = newTaskActorHost(registry)
	if m.maxDepth <= 0 {
		m.maxDepth = 4
	}
	...
	if err := m.importLegacyTasks(context.Background()); err != nil {
		m.reportTaskMigrationError(err)
	}
...
func (m *Manager) storageError() error {
	...
	if err := m.registry.err(); err != nil {
		return err
	}
	if m.actors == nil || m.actors.system == nil {
		return fmt.Errorf("task actor host is unavailable")
	}
	return nil
}
...
func (m *Manager) runStreamingTask(ctx context.Context, task TaskSnapshot, events chan<- model.StreamEvent) {
	defer close(events)
	ctx = model.WithRequestScope(ctx, model.RequestScope{TaskID: task.ID, ParentTaskID: task.ParentTaskID, ParentSessionID: task.ParentSessionID, Purpose: "worker_stream"})
...
func (m *Manager) contextLimitTokens() int {
	var cfg model.Config
	if reader, ok := m.model.(interface{ CurrentModelConfig() model.Config }); ok {
		cfg = reader.CurrentModelConfig()
	}
	general := 0
	if m.settings != nil {
		general = m.settings.CurrentSettings().UI.ContextLimitTokens
	}
	return model.ResolveContextLimitTokens(cfg, general)
}
...
func (m *Manager) runSession(ctx context.Context, sessionID, prompt string, disableTools bool) (string, int, *tokentracer.Usage, error) {
	...
	engine.BindContextLimitSource(m.contextLimitTokens)
...
func (m *Manager) toolRegistry(disableTools bool) *tool.Registry {
	...
	root, projectDir := "", ""
	if m != nil {
		root, projectDir = m.root, m.registry.projectDir
	}
	registry := newBaseToolRegistry(root, projectDir)
...
func newBaseToolRegistry(root, projectDir string) *tool.Registry {
	registry := tool.NewRegistry()
	readRoots := skill.DefaultRoots(root)
	if projectDir != "" {
		readRoots = append(readRoots, projectDir)
	}
...
func (u *streamingUI) OnModelRequestStart(id string) {
	_ = sendModelStreamEvent(u.ctx, u.events, model.StreamEvent{RequestStartID: id})
}
...
func (t *statusTool) Run(ctx context.Context, raw json.RawMessage) (string, error) {
	if err := t.manager.storageError(); err != nil {
		return "", err
	}
	if err := ctx.Err(); err != nil {
		return "", err
...
func TranscriptPath(root, sessionID string) string {
	...
	if err != nil || validateTaskID(sessionID) != nil {
		return ""
	}
	return filepath.Join(projectDir, "sessions", sessionID, "transcript.jsonl")
}
