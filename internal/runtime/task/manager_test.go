...
func newTestManager(t *testing.T, modelStreamer loop.ModelStreamer, cfg settings.Config, notifier Notifier) (*Manager, *session.JSONLStore, string) {
	t.Helper()
	root := t.TempDir()
	store, err := session.NewJSONLStore(taskProjectDirForTest(t, root))
	if err != nil {
...
func TestBaseToolRegistryDoesNotContainUpdateTodo(t *testing.T) {
	root := t.TempDir()
	registry := newBaseToolRegistry(root, taskProjectDirForTest(t, root))
...
func TestStreamEmitsDeltaBeforeModelDone(t *testing.T) {
	...
	case ev := <-stream.Events:
		if ev.RequestStartID == "" || ev.Usage != nil {
			t.Fatalf("missing request lifecycle before delta: %#v", ev)
		}
	case <-time.After(time.Second):
		t.Fatal("Stream() did not emit request start")
	}
	select {
	case ev := <-stream.Events:
...
func TestCompletedTaskRecoversFromActorJournalWithoutMetadata(t *testing.T) {
	...
	metaPath := filepath.Join(taskProjectDirForTest(t, root), tasksDirName, result.SessionID, "meta.json")
...
func TestTaskLifecycleEventFlowGolden(t *testing.T) {
	...
	journal, err := es.NewJSONLStore(filepath.Join(taskProjectDirForTest(t, root), "actors"), taskActorType)
	if err != nil {
...
func TestRunningTaskRecoversInterruptedFromActorJournalWithoutMetadata(t *testing.T) {
	...
	metaPath := filepath.Join(taskProjectDirForTest(t, root), tasksDirName, task.ID, "meta.json")
	if err := os.Remove(metaPath); err != nil {
		t.Fatalf("remove compatibility metadata: %v", err)
	}
