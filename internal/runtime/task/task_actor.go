...
func (a *taskActor) Receive(ctx *actor.Context, msg actor.Msg) {
	...
	case taskActorReplace:
		...
		mutation.Task.OutputPath = a.registry.outputPath(mutation.Task.ID)
...
func (a *taskActor) Restore(state json.RawMessage) error {
	...
	a.state.Task.OutputPath = a.registry.outputPath(a.state.Task.ID)
	return nil
}
...
func newTaskActorHost(registry taskRegistry) *taskActorHost {
	if registry.err() != nil {
		return nil
	}
	processes := newTaskProcessTable()
	updates := newTaskUpdateBroker()
	system := actor.NewSystem(filepath.Join(registry.projectDir, "actors"), actor.WithShards(1))
...
func (h *taskActorHost) list(ctx context.Context) ([]TaskSnapshot, error) {
	if h == nil || h.system == nil {
		return nil, fmt.Errorf("task actor host is unavailable")
	}
	if ctx == nil {
		ctx = context.Background()
	}
