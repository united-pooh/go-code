...
func NewEventStore(docDir, eventsDir string) (*EventStore, error) {
	...
	if err := preparePlanDirectory(docDir); err != nil {
		return nil, err
	}
...
func (s *EventStore) loadState(ctx context.Context, id PlanID) (esState, bool, error) {
	...
	// Projection location follows the configured directory, not a historical
	// path embedded in otherwise unchanged domain events.
	st.Doc.Path, err = s.pathFor(id)
	if err != nil {
		return esState{}, false, err
	}
	return *st, true, nil
...
// （跟随 internal/platform/config CAS writer 模式）；基线在锁外读取，保证先取得锁
// 的并发写入者被检测为冲突而不是被覆盖。
