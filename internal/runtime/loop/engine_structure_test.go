...
func TestEngineOwnsSmallIndependentExecutionState(t *testing.T) {
	...
	file, err := parser.ParseFile(fset, "engine.go", nil, parser.SkipObjectResolution)
	if err != nil {
	...
	if foundRunner {
		t.Fatal("legacy loop.SessionHost type must be removed")
	}
	if engineFields > 15 {
...
func TestLoopProductionFilesDoNotImportActorRuntime(t *testing.T) {
	...
	for _, entry := range entries {
		...
		for _, imp := range file.Imports {
			if imp.Path.Value == `"paw/internal/runtime/actor"` {
				t.Fatalf("%s imports internal/runtime/actor; loop.Engine must remain actor-independent", name)
			}
		}
	}
}
