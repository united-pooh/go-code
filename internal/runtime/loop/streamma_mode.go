...
func (runner *Engine) runStreamMATurn(ctx context.Context, input string, invocation streamMAInvocation) (msg message.Message, err error) {
	...
	if writeErr := runner.writeStreamMARoutingRationale(rationale); writeErr != nil {
	...
	if writeErr := runner.writeStreamMARunLogger(result.RunID, logger); writeErr != nil {
		runner.notifySystem("streamma-trace", "runlogger write failed: "+writeErr.Error())
	}
	if writeErr := runner.appendStreamMAEvidence(streamma.BuildRoutingEvidenceRecord(task, decision, logger)); writeErr != nil {
		runner.notifySystem("streamma-trace", "evidence write failed: "+writeErr.Error())
	}
	if graphKind == streamMAGraphReview {
		artifacts, artifactErr := review.WriteReviewArtifacts(ctx, review.ReviewArtifactsOptions{
		...
		runner.notifySystem("pipeline", fmt.Sprintf("wrote strict tree grading artifacts to %s (score=%d verdict=%s)", artifacts.ArtifactDir, artifacts.Assessment.AggregateScore, artifacts.Assessment.Verdict))
	}
...
func (runner *Engine) appendStreamMAEvidence(record streamma.RoutingEvidenceRecord) error {
	dir, err := runner.streamMAArtifactDir()
	if err != nil {
		return err
	}
...
func (runner *Engine) writeStreamMAJSONArtifact(fileName string, value interface{}) error {
	...
	return runner.writeStreamMAJSONArtifactInDir(dir, fileName, value)
}
...
func (runner *Engine) writeStreamMARunJSONArtifact(runID, fileName string, value interface{}) error {
	...
	return runner.writeStreamMAJSONArtifactInDir(filepath.Join(dir, "runs", streamma.SafeArtifactID(runID)), fileName, value)
}

func (runner *Engine) streamMAArtifactDir() (string, error) {
	root := ""
	if runner != nil {
		root = runner.workRoot
	}
	...
	if err != nil {
		return "", fmt.Errorf("resolve StreamMA artifacts: %w", err)
	}
	return filepath.Join(project, filepath.FromSlash(streamma.ArtifactDirectory)), nil
}
...
func streamMAReviewWorkspaceSnapshot(root string) string {
	...
	for _, rel := range []string{"go.mod", "README.md", "cmd/paw/main.go"} {
...
func (m *streamMATaskModel) wrapStream(ctx context.Context, invocation streamma.AgentInvocation, stream StreamMATaskStream, spanIndex int) <-chan model.StreamEvent {
	...
	go func() {
		...
		var usageMeter model.UsageAccumulator
		reported := make(map[string]bool)
		recordCall := func(requestID string, usage tokentracer.Usage, source string, known bool) {
			if m.tokenTracer == nil {
				return
			}
			provider, modelName := "", ""
			...
			agentID, role := strings.TrimSpace(invocation.AgentID), strings.TrimSpace(invocation.Role)
			m.tokenTracer.RecordAPICall(m.traceStageID, "", agentID, streamMATraceAgentName(agentID, role), provider, modelName, usage, map[string]any{
				"request_id": requestID, "source": source, "usage_known": known,
				"run_id": invocation.RunID, "role": role, "invocation_index": invocation.InvocationIndex, "session_id": stream.SessionID,
			})
		}
		recordUsage := func(usage model.Usage) {
			if usage.RequestID == "" {
				usage.RequestID = fmt.Sprintf("streamma-%s/%s/%d/%d", invocation.RunID, invocation.AgentID, invocation.InvocationIndex, invocation.Attempt)
			}
			previousUsage := tokentracer.UsageFromModelUsage(usageMeter.Total().ModelUsage())
			usageMeter.Observe(usage)
			total := usageMeter.CombinedUsage()
			lastUsage = &total
			currentUsage := tokentracer.UsageFromModelUsage(total)
			delta := currentUsage.Delta(previousUsage)
			if !reported[usage.RequestID] || !delta.Empty() {
				recordCall(usage.RequestID, delta, "streamma_task", true)
				reported[usage.RequestID] = true
			}
		}
		recordEvent := func(ev model.StreamEvent) {
			if ev.RequestStartID != "" {
				recordCall(ev.RequestStartID, tokentracer.Usage{}, "request_start", false)
			}
			if ev.Usage != nil {
		...
		for ev := range stream.Events {
			...
			if !sendStreamMAEvent(ctx, out, ev) {
				for remaining := range stream.Events {
					recordEvent(remaining)
				}
				return
			}
		}
