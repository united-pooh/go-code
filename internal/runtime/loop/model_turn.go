...
func (runner *Engine) runModelTurn(ctx context.Context, history []message.Message, turn *TurnState) (message.Message, error) {
	...
	for attempt := 0; attempt < maxLengthContinuationRequests; attempt++ {
		...
		events, err := runner.model.StreamMessage(runner.usageRequestContext(ctx, &state), modelMessages, tools)
		if err != nil {
...
func (runner *Engine) beginUsageRequest(state *turnState) {
	state.requestUsage = model.Usage{RequestID: rand.Text()}
	state.requestUsageKnown = false
	if runner.display != nil {
		_ = runner.display.Publish(DisplayEvent{Kind: DisplayModelRequestStart, RequestID: state.requestUsage.RequestID})
	}
	runner.recordTraceUsage(state.traceStageID, state.traceAgentID, tokentracer.Usage{}, map[string]any{
		"source": "request_start", "request_id": state.requestUsage.RequestID, "usage_known": false,
	})
}

func (runner *Engine) usageRequestContext(ctx context.Context, state *turnState) context.Context {
	scope := model.RequestScopeFromContext(ctx)
	runner.mu.RLock()
	scope.SessionID = runner.sessionID
	runner.mu.RUnlock()
	if owner, ok := TurnOwnerFromContext(ctx); ok {
		scope.TurnID = owner.TurnID
	}
	if state.requestPurpose != "" {
		scope.Purpose = state.requestPurpose
	} else if scope.Purpose == "" {
		scope.Purpose = "conversation"
	}
	return model.WithRequestScope(model.WithUsageRequestID(ctx, state.requestUsage.RequestID), scope)
}
...
func (runner *Engine) recordUsageEvent(state *turnState, usage model.Usage) {
	...
	state.requestUsage = model.MergeUsageSnapshot(state.requestUsage, usage)
	...
	runner.setCurrentUsage(state.requestUsage)
	runner.addSessionUsage(delta)
	runner.emitModelUsage(state.requestUsage)
	...
	runner.recordTraceUsage(state.traceStageID, state.traceAgentID, currentTrace.Delta(previousTrace), map[string]any{
		"source":      "model_stream",
		"request_id":  state.requestUsage.RequestID,
		"purpose":     state.requestPurpose,
		"usage_known": true,
	})
}
...
func usageTotalsFromUsage(usage model.Usage, known bool) tokenUsageTotals {
	...
	b := usage.Breakdown()
	return tokenUsageTotals{used: b.Total(), cache: b.CacheRead, creation: b.CacheCreation, output: b.Output, reasoning: b.Reasoning}
}

func usageFromTotals(t tokenUsageTotals) model.Usage {
	t = t.normalized()
	return (model.UsageBreakdown{Input: maxInt(0, t.used-t.output-t.cache-t.creation), Output: t.output, CacheRead: t.cache, CacheCreation: t.creation, Reasoning: t.reasoning}).ModelUsage()
}

func (t tokenUsageTotals) add(other tokenUsageTotals) tokenUsageTotals {
	return tokenUsageTotals{
		used:      t.used + other.used,
		cache:     t.cache + other.cache,
		output:    t.output + other.output,
		creation:  t.creation + other.creation,
...
func (t tokenUsageTotals) delta(previous tokenUsageTotals) tokenUsageTotals {
	return tokenUsageTotals{
		used:      t.used - previous.used,
		cache:     t.cache - previous.cache,
		output:    t.output - previous.output,
		creation:  t.creation - previous.creation,
		reasoning: t.reasoning - previous.reasoning,
	}
}

func (t tokenUsageTotals) normalized() tokenUsageTotals {
	t.creation = maxInt(0, t.creation)
	t.reasoning = maxInt(0, t.reasoning)
	if t.used < 0 {
		t.used = 0
	}
	if t.cache < 0 {
