...
// ArtifactDirectory is relative to the Paw project storage root, not the cwd.
const ArtifactDirectory = "artifacts/streamma-routing"
...
func RunLoggerArtifactPath() string {
	return filepath.ToSlash(filepath.Join(ArtifactDirectory, RunLoggerFileName))
}

func RunLoggerRunArtifactPath(runID string) string {
	return filepath.ToSlash(filepath.Join(ArtifactDirectory, "runs", SafeArtifactID(runID), RunLoggerFileName))
}
