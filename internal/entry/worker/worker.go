...
func Run(ctx context.Context, input io.Reader, output io.Writer, allowOutsideRead bool, sandboxLimits task.SandboxLimits) error {
...
func readTaskWorkerStart(decoder *json.Decoder) (task.WorkerMessage, task.WorkerRequest, appcore.WorkerContext, error) {
	var start task.WorkerMessage
	if err := decoder.Decode(&start); err != nil {
		return task.WorkerMessage{}, task.WorkerRequest{}, appcore.WorkerContext{}, fmt.Errorf("decode task worker.start: %w", err)
	}
	if start.Type != task.WorkerMessageStart {
		return task.WorkerMessage{}, task.WorkerRequest{}, appcore.WorkerContext{}, fmt.Errorf("task worker.start is required")
	}
	req := start.Request()
	if err := validateWorkerRequest(req); err != nil {
		return task.WorkerMessage{}, task.WorkerRequest{}, appcore.WorkerContext{}, err
	}
	return start, req, appcore.WorkerContext{
		WorkerMode:      true,
		Depth:           req.Depth,
...
func runWorkerTurn(ctx context.Context, req task.WorkerRequest, broker *workerMCPBroker, allowOutsideRead bool) task.WorkerResult {
	ctx = model.WithRequestScope(ctx, model.RequestScope{TaskID: req.TaskID, ParentTaskID: req.ParentTaskID, ParentSessionID: req.ParentSessionID, Purpose: "worker"})
	...
	app, err := appcore.BuildWorkspaceRuntime(ctx, appcore.WorkspaceRuntimeOptions{
		Root: root, SessionID: req.SessionID, Output: workerUI,
		AllowOutsideRead: allowOutsideRead,
		WorkerContext: appcore.WorkerContext{
			WorkerMode: true, Depth: req.Depth, MaxDepth: req.MaxDepth,
			ParentTaskID: req.TaskID, DisableMainTodo: true, MCPBroker: broker,
		},
	})
	if err != nil {
		return task.WorkerResult{TaskID: req.TaskID, Error: err.Error(), ExitCode: 1}
	...
	host, sessionID := app.SessionHost, app.SessionID
	...
	broker.SetUpdateHandler(func(snapshot coremcp.Snapshot) {
		...
		_ = host.ReplaceToolNamespace("mcp", tools)
	})
	broker.Update(broker.Snapshot())
	msg, runErr := host.RunTurn(ctx, req.Prompt)
	result.UsedTokens = host.ContextStats(1<<30, "").UsedTokens
...
func RunPool(ctx context.Context, input io.Reader, output io.Writer, allowOutsideRead bool, sandboxLimits task.SandboxLimits) error {
	if err := task.ApplyWorkerResourceLimits(sandboxLimits); err != nil {
		return fmt.Errorf("apply worker resource limits: %w", err)
	}
	if ctx == nil {
...
type workerUsageUI struct {
	...
	usageMeter model.UsageAccumulator
...
func (u *workerUsageUI) OnModelRequestStart(id string) {
	if u.emit != nil {
		u.emit(task.WorkerStreamEvent{RequestStartID: id})
	}
}

func (u *workerUsageUI) OnModelUsage(usage model.Usage) {
	u.mu.Lock()
	u.usageMeter.Observe(usage)
	structured := tokentracer.UsageFromModelUsage(u.usageMeter.Total().ModelUsage())
	u.usage = &structured
	u.mu.Unlock()
	if u.emit != nil {
		copied := usage
